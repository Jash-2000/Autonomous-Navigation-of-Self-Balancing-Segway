...
figure(1)
subtitle('Balancing Action')

subplot(2,2,1), impulse(sts_dist);   % Impulse reponse of distance
title('Regulatory Action')
xlabel('Time')
ylabel('Distance')

subplot(2,2,2), step(sts_dist);      % Step Response of distance
title('Servo Action')
xlabel('Time')
ylabel('Distance')

subplot(2,2,3), impulse(sts_angle);   % Impulse reponse of Balancing Angle
title('Regulatory Action')
xlabel('Time')
ylabel('Angular Dip in Cat-Pole')

subplot(2,2,4), step(sts_angle);      % Step Response of Balancing Angle
title('Servo Action')
xlabel('Time')
ylabel('Angular Dip in Cat-Pole')
...
for i = 1:length(arr)
    ...
    angle = evalin('base', 'angle.Data');

    x = zeros(1,length(rad));
    y = zeros(1,length(rad));

    for j = 1:length(rad)
        x(j) = x_last + rad(j)*cos(theta(j));
        y(j) = y_last + rad(j)*sin(theta(j));    
    end
...
figure(3)
plot(Time',Theta')
title('The Angle change in the bot trajectory')
figure(4)
plot(Time',Rad')
title('Distane travelled')
figure(5)
plot(Time',Angle')
title('Dip in the inverted pendulum')